...
const UploadFile = () => {
  ...
  const onSubmit = async (e) => {
    ...
    await axios.post(`${API_URL}files`, {
      ...
      users: users.map(user => user.id).concat([state.firebase.user.uid]),
  ...
  return (
    <form className='flex items-center mt-5 flex-col' onSubmit={onSubmit} autoComplete="off" >
      <h1 className="mt-5 mb-5 self-center font-bold leading-tight text-4xl text-blue-800" onClick={() => set()}>
        Upload a New File
      </h1>
      {loading ? <div></div> :
        <div className='flex grid grid-cols-1 md:grid-cols-2 w-full mt-10 px-5 ' >
          <div className="flex justify-center items-center w-full md:mr-5 flex-col" onClick={() => set()}>
              {selectedFile ?
            ...
            <div className='flex flex-col justify-center items-center my-5'>
              ...
              users.map(user => {
                return (
                  <button className={`bg-blue-200 mt-1 rounded-full`} key={user.uid}
                    onClick={() => {
                      setUsers(users.filter(function(el) { return el.uid != user.uid; }))
                    }}
                  >
                    <div className={'flex w-50  px-2 justify-center py-1 rounded-lg items-center '}>
        ...
        <div className='flex flex-col justify-center xl:px-24 lg:px-16 md:px-11 sm:px-24 px-11' >
            <SearchBarUsers setUsers={setUsers} setUsersList={setUsersList} setIsFocused={setIsFocused} isFocused={isFocused} usersList={usersList} users={users} />
           <label htmlFor="first_name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300 mt-3">File Name</label>
          <InputElement
            setElement={(e) =>
          ...
          <InputElement
            ...
            type={"text"}
          />
            <div className='flex justify-between items-center w-full' onClick={() => set()} >
...
const SearchBarUsers = ({ setUsers, setUsersList, usersList, users, setIsFocused, isFocused }) => {
  const [search, setSearch] = useState("")

  useEffect(() => {
    if (search.length > 1)
      axios.get(API_URL + "users/" + search).then(res => setUsersList(res.data))
    else
      setUsersList([])
  }, [search])
  ...
  return (
    <div className=''>
      <div className='flex flex-row justify-between '>
        <label htmlFor="first_name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Add Users</label>
        {isFocused && <div className="block mb-2 text-sm font-bold text-blue-700 dark:text-gray-300" onClick={() => setIsFocused(false)}>
        ...
        <div className='relative ' >
          <SearchBar
            onChange={(e) => { setIsFocused(true); setSearch(e.target.value)}}
            placeholder={"Search users"}
            value={search}
          />
          {isFocused &&
          <div id="dropdown" className="absolute z-20 w-full bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700">
                  ...
                  return (
                    <li key={user.uid}>
                      <a href="#" onClick={(e) => {
                        ...
                        else setUsers(users.filter(function(el) { return el.uid != user.uid; }))
                      }} user={user} className={`block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white ${found && "text-blue-700 bg-blue-100"}`}>{user.displayName}</a>
                    </li>
                )
              })
            }
